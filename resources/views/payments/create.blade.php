        ...
        <div class="mb-6 bg-blue-50 p-4 rounded-lg">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-green-900">{{ number_format($quote->paid_amount, 2, ',', ' ') }} GNF</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-600">Solde restant</p>
                    <p class="text-xl font-bold text-orange-900">{{ number_format($quote->remaining_amount, 2, ',', ' ') }} GNF</p>
                </div>
            </div>
        </div>

        <form action="{{ route('payments.store', $quote) }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    ...
                    @else
                        <p class="mt-1 text-xs text-gray-500">Maximum: {{ number_format($quote->remaining_amount, 2, ',', ' ') }} GNF</p>
                    @enderror
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="notes" 
                              ...
                              style="focus:ring-color: {{ $settings->primary_color ?? '#3b82f6' }};"
                              placeholder="Notes supplémentaires sur ce paiement...">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
