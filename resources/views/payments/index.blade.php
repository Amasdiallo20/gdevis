                    ...
                    <div class="bg-gradient-to-r from-orange-50 to-red-50 border-2 border-orange-200 rounded-lg p-4 shadow-md hover:shadow-lg transition-all duration-200 cursor-pointer transform hover:scale-105">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xl sm:text-2xl font-bold" style="color: {{ $settings->primary_color ?? '#3b82f6' }};">
                                    {{ number_format($totalRemainingAmount ?? 0, 2, ',', ' ') }} <span class="text-sm text-gray-500">GNF</span>
                                </p>
                            ...
                            <div class="ml-4">
                                <div class="w-12 h-12 rounded-full flex items-center justify-center" 
                                    ...
                                    <i class="fas fa-wallet text-xl" style="color: {{ $settings->primary_color ?? '#3b82f6' }};"></i>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
        <form method="GET" action="{{ route('payments.index') }}" class="space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                @if(request()->hasAny(['search', 'quote_id', 'client_id', 'date_from', 'date_to', 'payment_method']))
    ...
    <div class="p-6">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="table-row-hover">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <i class="fas fa-calendar-alt mr-2 text-gray-400"></i>{{ $payment->payment_date->format('d/m/Y') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <a href="{{ route('quotes.show', $payment->quote) }}" class="text-blue-600 hover:text-blue-800 font-bold transition-colors">
                                {{ $payment->quote->quote_number }}
                            </a>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $payment->quote->client->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">
                            <span class="text-lg" style="color: {{ $settings->primary_color ?? '#3b82f6' }};">{{ number_format($payment->amount, 2, ',', ' ') }}</span> <span class="text-xs text-gray-500">GNF</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $payment->payment_method_label }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $payment->reference ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-2">
                                <a href="{{ route('payments.print', $payment) }}" target="_blank" class="text-green-600 hover:text-green-800 p-2.5 rounded-lg hover:bg-green-50 transition-all duration-200 transform hover:scale-110" title="Imprimer le reçu">
                                    <i class="fas fa-receipt"></i>
                                </a>
                                <a href="{{ route('payments.edit', [$payment->quote, $payment]) }}" class="text-blue-600 hover:text-blue-800 p-2.5 rounded-lg hover:bg-blue-50 transition-all duration-200 transform hover:scale-110" title="Modifier">
        ...
        <div class="text-center py-16">
            <div class="inline-flex items-center justify-center w-20 h-20 rounded-full mb-4"
                 style="background: linear-gradient(135deg, {{ $settings->primary_color ?? '#3b82f6' }}20 0%, {{ $settings->secondary_color ?? '#1e40af' }}20 100%);">
                <i class="fas fa-money-bill-wave text-4xl" style="color: {{ $settings->primary_color ?? '#3b82f6' }};"></i>
            </div>
