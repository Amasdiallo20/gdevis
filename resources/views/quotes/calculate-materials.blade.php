...
@section('title', 'Calcul des Matériaux')
...
<div class="card-modern overflow-hidden">
    ...
    <div class="p-6">
        ...
        <form method="GET" action="{{ route('quotes.calculate-materials') }}" class="mb-6">
                ...
                <div class="sm:col-span-2">
                    ...
                    <select name="quote_id" id="quote_id" required
                        ...
                        @foreach($quotes as $quote)
                            <option value="{{ $quote->id }}" {{ request('quote_id') == $quote->id ? 'selected' : '' }}>
                                {{ $quote->quote_number }} - {{ $quote->client->name ?? 'N/A' }} ({{ $quote->date->format('d/m/Y') }})
                            </option>
                        @endforeach
                    </select>
                </div>
        ...
        @if($selectedQuote && $materials)
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                            <div>
                                <span class="font-semibold">Client :</span> {{ $selectedQuote->client->name ?? 'N/A' }}
                            </div>
                            <div>
                                <span class="font-semibold">Date :</span> {{ $selectedQuote->date->format('d/m/Y') }}
                            </div>
                        </div>
                    </div>
            ...
            <div class="mb-6">
                ...
                <div class="grid gap-3 mb-6" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); align-items: stretch;">
                    <div class="bg-white border-2 border-blue-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold whitespace-nowrap mb-1" style="color: {{ $settings->primary_color ?? '#3b82f6' }};">
                            {{ number_format($materials['total_cadre'], 3, ',', ' ') }}
                        </div>
                    ...
                    <div class="bg-white border-2 border-green-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-green-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_vento'], 3, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">barres</p>
                    </div>
                    <div class="bg-white border-2 border-purple-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-purple-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_sikane'], 3, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">barres</p>
                    </div>
                    <div class="bg-white border-2 border-orange-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-orange-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_moustiquaire'], 3, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">barres</p>
                    </div>
                    
                    <!-- Nouveaux matériaux ALU A82 -->
                    @if(isset($materials['total_fermeture_a82']) && $materials['total_fermeture_a82'] > 0)
                    <div class="bg-white border-2 border-cyan-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-cyan-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_fermeture_a82'], 0, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">Paire</p>
                    </div>
                    @endif
                    @if(isset($materials['total_roulette_a82']) && $materials['total_roulette_a82'] > 0)
                    <div class="bg-white border-2 border-pink-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-pink-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_roulette_a82'], 0, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">Paire</p>
                    </div>
                    @endif
                    @if(isset($materials['total_roulette_moustiquaire']) && $materials['total_roulette_moustiquaire'] > 0)
                    <div class="bg-white border-2 border-yellow-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-yellow-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_roulette_moustiquaire'], 0, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">Paire</p>
                    </div>
                    @endif
                    @if(isset($materials['total_equaire_moustiquaire']) && $materials['total_equaire_moustiquaire'] > 0)
                    <div class="bg-white border-2 border-indigo-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-indigo-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_equaire_moustiquaire'], 0, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">Paire</p>
                    </div>
                    @endif
                    @if(isset($materials['total_joint_vitrage_m']) && $materials['total_joint_vitrage_m'] > 0)
                    <div class="bg-white border-2 border-amber-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-amber-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_joint_vitrage_m'], 2, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">m</p>
                    </div>
                    @endif
                    @if(isset($materials['total_joint_moustiquaire_m']) && $materials['total_joint_moustiquaire_m'] > 0)
                    <div class="bg-white border-2 border-rose-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-rose-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_joint_moustiquaire_m'], 2, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">m</p>
                    </div>
                    @endif
                    @if(isset($materials['total_vitre']) && $materials['total_vitre'] > 0)
                    <div class="bg-white border-2 border-emerald-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-emerald-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_vitre'], 2, ',', ' ') }}
                        </div>
                ...
                <div class="grid gap-3 mb-6" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); align-items: stretch;">
                    ...
                    @if(isset($materials['total_fermeture_3rails']) && $materials['total_fermeture_3rails'] > 0)
                    <div class="bg-white border-2 border-cyan-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        <label class="block text-xs font-semibold text-gray-700 mb-2 whitespace-nowrap">Fermeture 3R</label>
                        <div class="text-2xl font-bold text-cyan-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_fermeture_3rails'], 0, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">Paire</p>
                    </div>
                    @endif
                    @if(isset($materials['total_roulette_vento_3rails']) && $materials['total_roulette_vento_3rails'] > 0)
                    <div class="bg-white border-2 border-lime-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-lime-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_roulette_vento_3rails'], 0, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">Paire</p>
                    </div>
                    @endif
                    @if(isset($materials['total_roulette_moustiquaire_3rails']) && $materials['total_roulette_moustiquaire_3rails'] > 0)
                    <div class="bg-white border-2 border-yellow-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        <label class="block text-xs font-semibold text-gray-700 mb-2 whitespace-nowrap">Roul. Moust. 3R</label>
                        <div class="text-2xl font-bold text-yellow-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_roulette_moustiquaire_3rails'], 0, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">Paire</p>
                    </div>
                    @endif
                    @if(isset($materials['total_equaire_moustiquaire_3rails']) && $materials['total_equaire_moustiquaire_3rails'] > 0)
                    <div class="bg-white border-2 border-violet-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-violet-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_equaire_moustiquaire_3rails'], 0, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">Paire</p>
                    </div>
                    @endif
                    @if(isset($materials['total_brosse_libanais_m']) && $materials['total_brosse_libanais_m'] > 0)
                    <div class="bg-white border-2 border-sky-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-sky-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_brosse_libanais_m'], 2, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">m</p>
                    </div>
                    @endif
                    @if(isset($materials['total_joint_vitrage_3rails_m']) && $materials['total_joint_vitrage_3rails_m'] > 0)
                    <div class="bg-white border-2 border-amber-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        <label class="block text-xs font-semibold text-gray-700 mb-2 whitespace-nowrap">Joint Vitrage 3R</label>
                        <div class="text-2xl font-bold text-amber-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_joint_vitrage_3rails_m'], 2, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">m</p>
                    </div>
                    @endif
                    @if(isset($materials['total_joint_moustiquaire_3rails_m']) && $materials['total_joint_moustiquaire_3rails_m'] > 0)
                    <div class="bg-white border-2 border-fuchsia-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-fuchsia-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_joint_moustiquaire_3rails_m'], 2, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">m</p>
                    </div>
                    @endif
                    @if(isset($materials['total_grillage_moustiquaire_3rails_m']) && $materials['total_grillage_moustiquaire_3rails_m'] > 0)
                    <div class="bg-white border-2 border-emerald-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        <label class="block text-xs font-semibold text-gray-700 mb-2 whitespace-nowrap">Grillage Moust. 3R</label>
                        <div class="text-2xl font-bold text-emerald-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_grillage_moustiquaire_3rails_m'], 2, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">m</p>
                    </div>
                    @endif
                    @if(isset($materials['total_vitre_3rails']) && $materials['total_vitre_3rails'] > 0)
                    <div class="bg-white border-2 border-slate-200 rounded-lg p-3.5 shadow-md flex flex-col justify-between">
                        ...
                        <div class="text-2xl font-bold text-slate-700 whitespace-nowrap mb-1">
                            {{ number_format($materials['total_vitre_3rails'], 2, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium">feuille</p>
                    </div>
                    @endif
                ...
                @if(isset($materials['total_cadre_porte']) && ($materials['total_cadre_porte'] > 0 || $materials['total_battant_porte'] > 0 || $materials['total_division'] > 0 || ($materials['total_brosse_a82_porte_m'] ?? 0) > 0 || ($materials['total_pomelles'] ?? 0) > 0 || ($materials['total_vitre_porte'] ?? 0) > 0 || ($materials['total_joint_vitrage_porte_m'] ?? 0) > 0))
                <h3 class="text-xl font-bold text-gray-900 mb-4 flex items-center">
                    <i class="fas fa-door-open mr-2" style="color: {{ $settings->primary_color ?? '#3b82f6' }};"></i>
                ...
                <div class="grid gap-2.5 mb-6" style="grid-template-columns: repeat(auto-fit, minmax(140px, max-content)); justify-content: start;">
                    @if(isset($materials['total_cadre_porte']) && $materials['total_cadre_porte'] > 0)
                    <div class="bg-white border-2 border-indigo-200 rounded-lg p-2.5 shadow-md flex flex-col min-w-[140px]">
                        ...
                        <div class="text-lg font-bold text-indigo-700 whitespace-nowrap">
                            {{ number_format($materials['total_cadre_porte'], 3, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 mt-0.5">barres</p>
                    </div>
                    @endif
                    @if(isset($materials['total_battant_porte']) && $materials['total_battant_porte'] > 0)
                    <div class="bg-white border-2 border-teal-200 rounded-lg p-2.5 shadow-md flex flex-col min-w-[140px]">
                        ...
                        <div class="text-lg font-bold text-teal-700 whitespace-nowrap">
                            {{ number_format($materials['total_battant_porte'], 3, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 mt-0.5">barres</p>
                    </div>
                    @endif
                    @if(isset($materials['total_division']) && $materials['total_division'] > 0)
                    <div class="bg-white border-2 border-pink-200 rounded-lg p-2.5 shadow-md flex flex-col min-w-[140px]">
                        ...
                        <div class="text-lg font-bold text-pink-700 whitespace-nowrap">
                            {{ number_format($materials['total_division'], 3, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 mt-0.5">barres</p>
                    </div>
                    @endif
                    @if(isset($materials['total_brosse_a82_porte_m']) && $materials['total_brosse_a82_porte_m'] > 0)
                    <div class="bg-white border-2 border-cyan-200 rounded-lg p-2.5 shadow-md flex flex-col min-w-[140px]">
                        ...
                        <div class="text-lg font-bold text-cyan-700 whitespace-nowrap">
                            {{ number_format($materials['total_brosse_a82_porte_m'], 2, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 mt-0.5">m</p>
                    </div>
                    @endif
                    @if(isset($materials['total_pomelles']) && $materials['total_pomelles'] > 0)
                    <div class="bg-white border-2 border-purple-200 rounded-lg p-2.5 shadow-md flex flex-col min-w-[140px]">
                        ...
                        <div class="text-lg font-bold text-purple-700 whitespace-nowrap">
                            {{ number_format($materials['total_pomelles'], 0, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 mt-0.5">unité</p>
                    </div>
                    @endif
                    @if(isset($materials['total_vitre_porte']) && $materials['total_vitre_porte'] > 0)
                    <div class="bg-white border-2 border-emerald-200 rounded-lg p-2.5 shadow-md flex flex-col min-w-[140px]">
                        ...
                        <div class="text-lg font-bold text-emerald-700 whitespace-nowrap">
                            {{ number_format($materials['total_vitre_porte'], 2, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 mt-0.5">feuille</p>
                    </div>
                    @endif
                    @if(isset($materials['total_joint_vitrage_porte_m']) && $materials['total_joint_vitrage_porte_m'] > 0)
                    <div class="bg-white border-2 border-amber-200 rounded-lg p-2.5 shadow-md flex flex-col min-w-[140px]">
                        ...
                        <div class="text-lg font-bold text-amber-700 whitespace-nowrap">
                            {{ number_format($materials['total_joint_vitrage_porte_m'], 2, ',', ' ') }}
                        </div>
                        <p class="text-xs text-gray-500 mt-0.5">m</p>
                    </div>
                    @endif
            ...
            <div class="mb-6">
                <h3 class="text-xl font-bold text-gray-900 mb-4 flex items-center">
                    <i class="fas fa-window-maximize mr-2" style="color: {{ $settings->primary_color ?? '#3b82f6' }};"></i>
                            ...
                            <tr class="table-row-hover">
                                ...
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-cyan-700">
                                        {{ isset($detail['fermeture_3rails']) ? number_format($detail['fermeture_3rails'], 0, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">Paire</p>
                                </td>
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-lime-700">
                                        {{ isset($detail['roulette_vento_3rails']) ? number_format($detail['roulette_vento_3rails'], 0, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">Paire</p>
                                </td>
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-yellow-700">
                                        {{ isset($detail['roulette_moustiquaire_3rails']) ? number_format($detail['roulette_moustiquaire_3rails'], 0, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">Paire</p>
                                </td>
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-violet-700">
                                        {{ isset($detail['equaire_moustiquaire_3rails']) ? number_format($detail['equaire_moustiquaire_3rails'], 0, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">Paire</p>
                                </td>
                                @if(isset($detail['brosse_libanais_m']) && $detail['brosse_libanais_m'] > 0)
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-sky-700">
                                        {{ number_format($detail['brosse_libanais_m'], 2, ',', ' ') }}
                                    </span>
                                ...
                                @endif
                                @if(isset($detail['joint_vitrage_3rails_m']) && $detail['joint_vitrage_3rails_m'] > 0)
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-amber-700">
                                        {{ number_format($detail['joint_vitrage_3rails_m'], 2, ',', ' ') }}
                                    </span>
                                    <p class="text-xs text-gray-400">m</p>
                                </td>
                                @else
                                ...
                                @endif
                                @if(isset($detail['joint_moustiquaire_3rails_m']) && $detail['joint_moustiquaire_3rails_m'] > 0)
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-fuchsia-700">
                                        {{ number_format($detail['joint_moustiquaire_3rails_m'], 2, ',', ' ') }}
                                    </span>
                                    <p class="text-xs text-gray-400">m</p>
                                </td>
                                @else
                                ...
                                @endif
                                @if(isset($detail['grillage_moustiquaire_3rails_m']) && $detail['grillage_moustiquaire_3rails_m'] > 0)
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-emerald-700">
                                        {{ number_format($detail['grillage_moustiquaire_3rails_m'], 2, ',', ' ') }}
                                    </span>
                                    <p class="text-xs text-gray-400">m</p>
                                </td>
                                @else
                                ...
                                @endif
                                @if(isset($detail['vitre_3rails']) && $detail['vitre_3rails'] > 0)
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-slate-700">
                                        {{ number_format($detail['vitre_3rails'], 2, ',', ' ') }}
                                    </span>
                                ...
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-cyan-700">
                                        {{ isset($detail['fermeture_a82']) ? number_format($detail['fermeture_a82'], 0, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">Paire</p>
                                </td>
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-pink-700">
                                        {{ isset($detail['roulette_a82']) ? number_format($detail['roulette_a82'], 0, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">Paire</p>
                                </td>
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-yellow-700">
                                        {{ isset($detail['roulette_moustiquaire']) ? number_format($detail['roulette_moustiquaire'], 0, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">Paire</p>
                                </td>
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-indigo-700">
                                        {{ isset($detail['equaire_moustiquaire']) ? number_format($detail['equaire_moustiquaire'], 0, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">Paire</p>
                                </td>
                                @if(isset($detail['joint_vitrage_m']) && $detail['joint_vitrage_m'] > 0)
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-amber-700">
                                        {{ number_format($detail['joint_vitrage_m'], 2, ',', ' ') }}
                                    </span>
                                    <p class="text-xs text-gray-400">m</p>
                                </td>
                                @else
                                ...
                                @endif
                                @if(isset($detail['joint_moustiquaire_m']) && $detail['joint_moustiquaire_m'] > 0)
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-rose-700">
                                        {{ number_format($detail['joint_moustiquaire_m'], 2, ',', ' ') }}
                                    </span>
                                    <p class="text-xs text-gray-400">m</p>
                                </td>
                                @else
                                ...
                                @endif
                                @if(isset($detail['vitre']) && $detail['vitre'] > 0)
                                <td class="px-2 py-4 whitespace-nowrap text-center">
                                    <span class="text-xs font-semibold text-emerald-700">
                                        {{ number_format($detail['vitre'], 2, ',', ' ') }}
                                    </span>
                                    <p class="text-xs text-gray-400">feuille</p>
                                </td>
                                @else
                            ...
                            <tr class="table-row-hover">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-indigo-100 text-indigo-800">
                                        {{ $detail['nombre_portes'] }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full {{ $detail['type'] === '1_battant' ? 'bg-teal-100 text-teal-800' : 'bg-pink-100 text-pink-800' }}">
                                        {{ $detail['type'] === '1_battant' ? '1 BATTANT' : '2 BATTANTS' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-bold text-indigo-700">
                                        {{ number_format($detail['cadre_porte'], 3, ',', ' ') }}
                                    </span>
                                    <p class="text-xs text-gray-400">barres</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-bold text-teal-700">
                                        {{ number_format($detail['battant_porte'], 3, ',', ' ') }}
                                    </span>
                                    <p class="text-xs text-gray-400">barres</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-bold text-pink-700">
                                        {{ number_format($detail['division'], 3, ',', ' ') }}
                                    </span>
                                    <p class="text-xs text-gray-400">barres</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-semibold text-cyan-700">
                                        {{ isset($detail['brosse_a82_m']) ? number_format($detail['brosse_a82_m'], 2, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">m</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-semibold text-purple-700">
                                        {{ isset($detail['pomelles']) ? number_format($detail['pomelles'], 0, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">unité</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-semibold text-emerald-700">
                                        {{ isset($detail['vitre_m2']) ? number_format($detail['vitre_m2'], 2, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">feuille</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-semibold text-amber-700">
                                        {{ isset($detail['joint_vitrage_m']) ? number_format($detail['joint_vitrage_m'], 2, ',', ' ') : '0' }}
                                    </span>
                                    <p class="text-xs text-gray-400">m</p>
                                </td>
                                    <p class="text-xs text-gray-400">feuille</p>
        ...
        @elseif($selectedQuote)
            <div class="bg-yellow-50 border-2 border-yellow-200 rounded-lg p-6">
                ...
                @if($materials && isset($materials['debug']))
                <div class="mt-4 p-4 bg-white rounded border border-yellow-300">
                    ...
                    <p class="text-xs text-gray-600 mb-1">Total lignes: {{ $materials['debug']['total_lines'] }}</p>
                    <p class="text-xs text-gray-600 mb-3">Lignes produits: {{ $materials['debug']['product_lines'] }}</p>
                    
                    @if(isset($materials['debug']['lines_checked']) && count($materials['debug']['lines_checked']) > 0)
                    <div class="mt-3">
                        <p class="text-xs font-semibold text-gray-700 mb-2">Détails des lignes produits :</p>
                        <div class="space-y-2">
                            @foreach($materials['debug']['lines_checked'] as $lineInfo)
                            <div class="p-2 bg-gray-50 rounded text-xs">
                                <p><strong>Ligne #{{ $lineInfo['id'] }}:</strong> {{ $lineInfo['product_name'] }}</p>
                                <p class="text-gray-600">Description: {{ $lineInfo['description'] ?? 'N/A' }}</p>
                                <p class="text-gray-600">Contient "fenêtre": 
                                    <span class="{{ $lineInfo['has_fenetre'] ? 'text-green-600 font-bold' : 'text-red-600' }}">
                                        {{ $lineInfo['has_fenetre'] ? 'Oui' : 'Non' }}
                                    </span>
                                </p>
                                <p class="text-gray-600">Contient "porte": 
                                    <span class="{{ isset($lineInfo['has_porte']) && $lineInfo['has_porte'] ? 'text-green-600 font-bold' : 'text-red-600' }}">
                                        {{ isset($lineInfo['has_porte']) && $lineInfo['has_porte'] ? 'Oui' : 'Non' }}
                                    </span>
                                </p>
                                <p class="text-gray-600">Dimensions valides: 
                                    <span class="{{ $lineInfo['has_dimensions'] ? 'text-green-600 font-bold' : 'text-red-600' }}">
                                        {{ $lineInfo['has_dimensions'] ? 'Oui' : 'Non' }}
                                    </span>
                                    @if(!$lineInfo['has_dimensions'])
                                        (Largeur: {{ $lineInfo['width'] ?? '0' }} cm, Hauteur: {{ $lineInfo['height'] ?? '0' }} cm)
                                    @endif
                                </p>
                            </div>
                            @endforeach
