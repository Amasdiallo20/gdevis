        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 mb-6">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-100 shadow-sm">
                <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center">
                    <i class="fas fa-address-card mr-2" style="color: {{ $settings->primary_color ?? '#3b82f6' }};"></i>
                ...
                <dl class="space-y-4">
                    <div class="flex items-start">
                        ...
                        <dd class="text-sm font-bold text-gray-900">{{ $client->name }}</dd>
                    </div>
                    @if($client->email)
                    <div class="flex items-start">
                        <dt class="text-sm font-semibold text-gray-600 w-28 flex-shrink-0">
                        ...
                        <dd class="text-sm text-gray-900 break-words">{{ $client->email }}</dd>
                    </div>
                    @endif
                    @if($client->phone)
                    <div class="flex items-start">
            ...
            <div class="bg-gradient-to-br from-purple-50 to-pink-50 p-6 rounded-xl border border-purple-100 shadow-sm">
                <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center">
                    <i class="fas fa-map-marker-alt mr-2" style="color: {{ $settings->primary_color ?? '#3b82f6' }};"></i>
                    Adresse
                </h3>
                <dl class="space-y-4">
                    @if($client->address)
                    ...
                    <div class="flex items-start">
                        ...
                        <dd class="text-sm text-gray-900">{{ $client->city }}</dd>
                    </div>
                    @endif
                    ...
                    <div class="flex items-start">
                        ...
                        <dd class="text-sm text-gray-900">{{ $client->postal_code }}</dd>
                    </div>
                    @endif
                    @if($client->country)
                    <div class="flex items-start">
                        ...
                        <dd class="text-sm text-gray-900">{{ $client->country }}</dd>
                    </div>
                    @endif
                </dl>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="table-row-hover">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @endphp
                                <span class="px-3 py-1.5 inline-flex text-xs leading-5 font-bold rounded-lg shadow-sm border {{ $status['class'] }}">
                                    {{ $status['label'] }}
                                </span>
                            </td>
